---
title: "Twitter Coverage of the Research Support Community Day 2018"
...
rscday2018 <- readRDS("data/rscday2018.rds")
```
...
rscday2018 <- search_tweets("#rscday2018", 10000)
saveRDS(rscday2018, "data/rscday2018.rds")
```
...
rscday2018 %>%
  ...
  ggplot(aes(reorder(tags, n), n)) +
    ...
    labs(x = "hashtag", y = "count",
         title = "Hashtags used during rscday2018",
...
rscday2018 %>%
  ...
  ggplot(aes(date, n)) + geom_col(fill = "skyblue3") +  
    labs(x = "Date", y = "Tweets", title = "#rscday2018 tweets per day") +
    theme(axis.text = element_text(size = 12),
          axis.title = element_text(size = 12))
```

## Tweets by day and time
Filtered for date February 12, Melbourne time.
...
rscday2018 %>%
  ...
  summarise(count = n()) %>%
  filter(date >= as_date("2018-02-12"), date <= as_date("2018-02-12")) %>%
  ggplot(aes(hour, count)) +
    geom_col(fill = "skyblue3") +
    facet_grid(strftime(date, "%b %d") ~ .) +
    labs(x = "Hour", y = "Tweets", title = "#rscday2018 tweets by time of day") +
    theme(axis.text = element_text(size = 12),
          axis.title = element_text(size = 12))
```
...
rscday2018 %>%
  ...
  ggplot(aes(reorder(screen_name, n), n)) +
    ...
    labs(x = "Screen Name", y = "Tweets",
         title = "#rscday2018 tweets by user",
...
rscday2018 %>%
  ...
  ggplot(aes(reorder(source, n), n)) +
    ...
    labs(x = "Source", y = "Tweets",
         title = "#rscday2018 tweets by source",
...
```{r reply-network}
rscday2018_replies <- rscday2018 %>%
  filter(!is.na(in_reply_to_status_screen_name)) %>%
  select(screen_name, in_reply_to_status_screen_name) %>%
  graph.data.frame(directed = TRUE)

V(rscday2018_replies)$label <- V(rscday2018_replies)$name
V(rscday2018_replies)$id    <- V(rscday2018_replies)$name

write_graph(rscday2018_replies, file = "data/rscday2018_replies.graphml",
            format = "graphml")
```
...
```{r mentions-network}
rscday2018_mentions <- rscday2018 %>%
...
V(rscday2018_mentions)$label <- V(rscday2018_mentions)$name
V(rscday2018_mentions)$id    <- V(rscday2018_mentions)$name

write_graph(rscday2018_mentions, file = "data/rscday2018_mentions.graphml",
            format = "graphml")
```
...
rscday2018 %>%
  ...
  ggplot(aes(is_retweet, n)) + geom_col(fill = "skyblue3") +
    labs(x = "Is retweet", y = "Tweets",
         title = "#rscday2018 tweets by retweet status") +
    theme(axis.text = element_text(size = 12),
          axis.title = element_text(size = 12))
```
...
rscday2018 %>%
  ggplot(aes(retweet_count)) +
    geom_histogram(bins = max(rscday2018$retweet_count), fill = "skyblue3") +
    labs(x = "Retweet count", y = "Tweets",
         title = "#rscday2018 distribution of retweets per tweet") +
    theme(axis.text = element_text(size = 12),
          axis.title = element_text(size = 12))
```
...
rscday2018 %>%
  ...
  ggplot(aes(has_favorite, n)) + geom_col(fill = "skyblue3") +
    labs(x = "Has favorite", y = "Tweets",
         title = "#rscday2018 tweets by favorited status") +
    theme(axis.text = element_text(size = 12),
          axis.title = element_text(size = 12))
```
...
rscday2018 %>%
  ggplot(aes(favorite_count)) +
    geom_histogram(bins = max(rscday2018$favorite_count), fill = "skyblue3") +
    labs(x = "Favorite count", y = "Tweets",
         title = "#rscday2018 distribution of favorites per tweet") +
    theme(axis.text = element_text(size = 12),
          axis.title = element_text(size = 12))
```
...
rscday2018 %>%
  ...
  ggplot(aes(is_quote_status, n)) +
    ...
    labs(x = "Is quote", y = "Tweets",
         title = "#rscday2018 tweets by quote status") +
    theme(axis.text = element_text(size = 12),
          axis.title = element_text(size = 12))
```
...
rscday2018 %>%
  ...
  ggplot(aes(n)) + geom_histogram(bins = 10, fill = "skyblue3") +
    labs(x = "Quote count", y = "Tweets",
         title = "#rscday2018 distribution of quotes per tweet") +
...
rscday2018 %>%
  ...
  arrange(desc(n)) %>%
#  slice(1:10) %>%
  inner_join(select(rscday2018, screen_name, quote_status_id,
                    is_retweet, text)) %>%
  filter(is_retweet == FALSE) %>%
...
rscday2018 %>%
  ...
  ggplot(aes(has_media, n)) +
    ...
    labs(x = "Has media", y = "Tweets",
         title = "#rscday2018 tweets by media status") +
    theme(axis.text = element_text(size = 12),
          axis.title = element_text(size = 12))
```
...
![](`r rscday2018_media[1, "media_url"]`)
![](`r rscday2018_media[2, "media_url"]`)
![](`r rscday2018_media[3, "media_url"]`)
![](`r rscday2018_media[4, "media_url"]`)
![](`r rscday2018_media[5, "media_url"]`)
...
rscday2018 %>%
  ...
  select(word) %>%
  filter(!word %in% c("rscday2018", "https", "t.co", "amp"),
         !word %in% tolower(rscday2018$screen_name),
         !grepl("^\\d+$", word)) %>%
